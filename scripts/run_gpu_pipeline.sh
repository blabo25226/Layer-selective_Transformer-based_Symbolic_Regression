...
for seed in $SEEDS; do
  LTSR_PHASE_TAG="seed${seed}" "${PY_CMD[@]}" scripts/phase6_noise_sweep.py \
    --noise $NOISE --data-root results/synthetic --tag diverse_gpu \
...
if [ "$PYSR" = "1" ]; then
  "${PY_CMD[@]}" scripts/phase8_lodo.py --epochs "$EPOCHS" --with-pysr --pysr-iters 40
else
  "${PY_CMD[@]}" scripts/phase8_lodo.py --epochs "$EPOCHS"
fi
