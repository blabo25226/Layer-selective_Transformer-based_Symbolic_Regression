...
$$
f^*=\underset{f\in\mathcal{F}}{\operatorname{argmin}}
\left[
\frac{1}{N}\sum_{i=1}^{N}\bigl(y_i-f(\mathbf{x}_i)\bigr)^2
+\lambda C(f)
\right]
$$
...
$$
R(f)=-\operatorname{NMSE}(f)-\lambda C(f)
$$
...
| **DREAM4** | 正解ネットワーク付きの人工遺伝子発現データを使うGRN推定ベンチマーク。 |
| **Encoder** | Transformerのうち、入力された数値点集合を内部表現へ変換する側。 |
| **Fine-tuning / FT** | 既に学習したモデルを、目的に合う少量のデータで追加学習すること。 |
| **Finite difference / FD** | 隣り合う時点の値の差から変化率を近似する方法。有限差分。 |
| **Generalization** | 学習に使っていないデータでも正しく働くこと。汎化。 |
| **GEO** | 公開遺伝子発現データベース Gene Expression Omnibus。本研究ではGSE112372を利用した。 |
| **GNW** | GeneNetWeaver。DREAM系の人工GRNと発現データを生成するソフトウェア。 |
| **GPU** | 大量の行列計算を並列処理する装置。深層学習をCPUより高速に実行できる。 |
| **GRN** | Gene Regulatory Network。遺伝子同士の活性化・抑制関係を表すネットワーク。 |
| **Hill式** | 遺伝子制御の飽和やスイッチらしい応答を表す代表的な数式。 |
| **ID / OOD** | IDは学習時に近い範囲、OODは学習時から外れた範囲での評価。 |
...
| **NeSymReS** | 数値点集合から数式を生成する、事前学習済みTransformer型SR。 |
| **NMSE** | 誤差をデータのばらつきで割った指標。0に近いほどよい。 |
...
| **PySR** | 進化計算で数式を探索するSRライブラリ。NeSymReSとの主要比較対象。 |
| **R²** | 予測がデータの変動をどれだけ説明できたかを表す指標。1に近いほどよい。 |
| **Regulator** | 標的遺伝子の発現に影響を与える遺伝子・転写因子。制御因子。 |
| **Repressilator** | 3遺伝子が環状に抑制し合う人工遺伝子回路。振動を起こし得る。 |
| **Seed** | 乱数の初期値。同じseedなら同じランダム処理を再現しやすい。 |
| **SBML** | 生物学的モデルを機械可読な形で交換するための標準形式。 |
| **Symbolic recovery** | 予測値だけでなく、正解と同じ数式構造を回復できたかという評価。 |
| **Symbolic Regression / SR** | データを説明する、人間が読める数式そのものを探索する回帰。 |
| **Token** | 数式をモデルが扱える小単位へ分けたもの。変数、演算子、括弧など。 |
| **TPSR** | Transformerの候補確率を使い、MCTSで数式を先読み探索する手法。 |
...
| **Variable F1** | 真の式に必要な変数を、予測式がどの程度過不足なく含むかを測る値。 |
...
{\sum_{i=1}^{N}(y_i-\bar y)^2+\varepsilon}
$$
...
$$
\frac{dx_2}{dt}
=-0.6x_2+\frac{2.5}{x_1^4+x_2/x_2}
=-0.6x_2+\frac{2.5}{x_1^4+1}
$$
...
$$
\widehat{\frac{dG_1}{dt}}
=-0.006125\tan(0.896395G_1-0.563921)
$$
...
$$
\widehat{\frac{d\,\mathrm{CCL5}}{dt}}
=0.0002435\,
\frac{x_1+x_2+x_3+13251.04}{x_1-2.0272}
$$

$$
\widehat{\frac{d\,\mathrm{CD40}}{dt}}
=\left[\cos(x_1+x_2-x_3)-0.06879\right]^2
$$

$$
\widehat{\frac{d\,\mathrm{IFNB1}}{dt}}
=\frac{0.62524x_1}{(0.40253x_2-x_3)^2}
$$
...
3. **Phase 5本比較**：同じseed集合でtop/random/bottom/fullを反復し、paired差とt信頼区間を求める。
